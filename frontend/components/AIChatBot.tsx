...
export default function AIChatBot() {
    ...
    return (
        // Fixed container wraps everything and anchors it securely in the screen corner
        <div className="fixed bottom-6 right-6 z-50 font-sans text-left flex flex-col items-end gap-4">

            {/* Main Conversational Chat Panel Drawer */}
            <div
                className={`w-[360px] sm:w-[400px] h-[500px] bg-white rounded-2xl shadow-2xl border border-slate-200/80 flex flex-col overflow-hidden transition-all duration-300 transform origin-bottom-right ${isOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 translate-y-10 pointer-events-none"
                    }`}
            >
                {/* Panel Header */}
                ...
                <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-200/80 flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="p-2.5 bg-[#2563eb] text-white rounded-xl hover:bg-[#1d4ed8] transition-colors duration-150 disabled:opacity-40 disabled:hover:bg-[#2563eb] focus:outline-none shadow-sm flex-shrink-0"
                    >
            ...
            {!isOpen && (
                <button
                    ...
                    className="w-14 h-14 bg-[#2563eb] hover:bg-[#1d4ed8] text-white rounded-full shadow-xl flex items-center justify-center transition-all duration-300 transform hover:scale-105 group flex-shrink-0"
                >
